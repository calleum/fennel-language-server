...
pub(crate) fn rope_range(rope: &Rope, lsp_range: Range) -> Result<std::ops::Range<usize>> {
...
pub(crate) fn position_to_char_idx(rope: &Rope, position: Position) -> Result<usize> {
    let start_char =
        rope.try_line_to_char(position.line as usize).map_err(|_err| Error::invalid_request())?;
    let utf16_cu = rope.char_to_utf16_cu(start_char) + position.character as usize;
    Ok(rope.utf16_cu_to_char(utf16_cu))
}

pub(crate) fn position_to_byte_offset(rope: &Rope, position: Position) -> Result<u32> {
    let start_char =
        rope.try_line_to_char(position.line as usize).map_err(|_err| Error::invalid_request())?;
    let utf16_cu = rope.char_to_utf16_cu(start_char) + position.character as usize;
...
pub(crate) fn byte_offset_to_position(rope: &Rope, offset: usize) -> Result<Position> {
    let line = rope.try_byte_to_line(offset).map_err(|_err| Error::invalid_request())?;
